...
QString ScriptManager::logException(const ScriptValuePointer& exception) {
    ...
    _engine(newScriptEngine(this))
{
    ...
    connect(this, &ScriptManager::unhandledException, this, [this](const ScriptValuePointer& err) {
...
static ScriptValuePointer scriptableResourceToScriptValue(ScriptEngine* engine,
                                                    const ScriptableResourceRawPtr& resource) {
    if (!resource) {
        return ScriptValuePointer();  // probably shutting down
    }
...
static void scriptableResourceFromScriptValue(const ScriptValuePointer& value, ScriptableResourceRawPtr& resource) {
 ...
 */
static ScriptValuePointer createScriptableResourcePrototype(ScriptManagerPointer manager) {
...
void externalResourceBucketFromScriptValue(const ScriptValuePointer& object, ExternalResource::Bucket& out) {
...
void ScriptManager::init() {
    ...
    ScriptValuePointer xmlHttpRequestConstructorValue = scriptEngine->newFunction(XMLHttpRequestClass::constructor);
    scriptEngine->globalObject()->setProperty("XMLHttpRequest", xmlHttpRequestConstructorValue);

    ScriptValuePointer webSocketConstructorValue = scriptEngine->newFunction(WebSocketClass::constructor);
    ...
    scriptEngine->globalObject()->setProperty("Kute", scriptEngine->newFunction([](ScriptContext* context, ScriptEngine* engine) -> ScriptValuePointer {
...
void ScriptManager::removeEventHandler(const EntityItemID& entityID, const QString& eventName, ScriptValuePointer handler) {
    if (QThread::currentThread() != thread()) {
        ...
        QMetaObject::invokeMethod(this, "removeEventHandler",
                                  ...
                                  Q_ARG(ScriptValuePointer, handler));
        return;
    }
#ifdef THREAD_DEBUGGING
    ...
    // ScriptValuePointer does not have operator==(), so we can't use QList::removeOne and friends. So iterate.
...
QObject* ScriptManager::setupTimerWithInterval(const ScriptValuePointer& function, int intervalMS, bool isSingleShot) {
...
QString ScriptManager::_requireResolve(const QString& moduleId, const QString& relativeTo) {
    ...
    auto throwResolveError = [&](const ScriptValuePointer& error) -> QString {
...
ScriptValuePointer ScriptManager::currentModule() {
    if (!_engine->IS_THREADSAFE_INVOCATION(__FUNCTION__)) {
...
bool ScriptManager::registerModuleWithParent(const ScriptValuePointer& module, const ScriptValuePointer& parent) {
...
ScriptValuePointer ScriptManager::newModule(const QString& modulePath, const ScriptValuePointer& parent) {
...
ScriptValuePointer ScriptManager::instantiateModule(const ScriptValuePointer& module, const QString& sourceCode) {
...
ScriptValuePointer ScriptManager::require(const QString& moduleId) {
    ...
    auto throwModuleError = [&](const QString& modulePath, const ScriptValuePointer& error) {
    ...
    cacheMeta->setProperty(modulePath, ScriptValuePointer());
...
void ScriptManager::include(const QStringList& includeFiles, ScriptValuePointer callback) {
    if (!_engine->IS_THREADSAFE_INVOCATION(__FUNCTION__)) {
        return;
    }
    if (isStopped()) {
...
void ScriptManager::forwardHandlerCall(const EntityItemID& entityID, const QString& eventName, ScriptValueList eventHandlerArgs) {
    if (QThread::currentThread() != thread()) {
    ...
    if (!handlersForEvent.isEmpty()) {
        for (int i = 0; i < handlersForEvent.count(); ++i) {
            ...
            callWithEnvironment(handler.definingEntityIdentifier, handler.definingSandboxURL, handler.function, ScriptValuePointer(), eventHandlerArgs);
        }
    }
}
...
void ScriptManager::entityScriptContentAvailable(const EntityItemID& entityID, const QString& scriptOrURL, const QString& contents, bool isURL, bool success , const QString& status) {
    ...
    ScriptValuePointer testConstructor, exception;
    ...
    ScriptValuePointer entityScriptConstructor, entityScriptObject;
...
void ScriptManager::doWithEnvironment(const EntityItemID& entityID, const QUrl& sandboxURL, std::function<void()> operation) {
    ...
    ScriptValuePointer oldData = this->globalObject().property("debugEntityID");
...
void ScriptManager::callWithEnvironment(const EntityItemID& entityID, const QUrl& sandboxURL, ScriptValuePointer function, ScriptValuePointer thisObject, ScriptValueList args) {
    auto operation = [&]() {
...
void ScriptManager::callEntityScriptMethod(const EntityItemID& entityID, const QString& methodName, const QStringList& params, const QUuid& remoteCallerID) {
    ...
    if (isEntityScriptRunning(entityID)) {
        ...
        ScriptValuePointer entityScript = details.scriptObject;  // previously loaded
...
QString ScriptManager::formatException(const ScriptValuePointer& exception, bool includeExtendedDetails) {
    if (!_engine->IS_THREADSAFE_INVOCATION(__FUNCTION__)) {
        return QString();
    }
